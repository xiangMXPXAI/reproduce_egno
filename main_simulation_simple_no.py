...
from training_utils import ensure_exp_dir, init_history, namespace_to_dict, save_config, save_history, save_summary
import os
...
def main():
    ...
    model_save_path = os.path.join(exp_dir, 'saved_model.pth')
    print(f'Model saved to {model_save_path}')
    early_stopping = EarlyStopping(patience=50, verbose=True, path=model_save_path)

    results = init_history(
        args,
        task_name='nbody_simulation',
        metric_note='loss is MSE on the last predicted timestep; training objective is mean MSE over all predicted timesteps.'
    )
    best_val_loss = 1e8
    best_test_loss = 1e8
    best_epoch = 0
    ...
    for epoch in range(0, args.epochs):
        ...
        results['train'].append({'epoch': epoch, 'loss': train_loss, 'lr': optimizer.param_groups[0]['lr']})
        if epoch % args.test_interval == 0:
            ...
            eval_record = {'epoch': epoch, 'val_loss': val_loss, 'test_loss': test_loss}
            results['eval'].append(eval_record)
            ...
            early_stopping(val_loss, model, checkpoint=checkpoint)
            save_history(results, exp_dir)
            if early_stopping.early_stop:
                print("Early Stopping.")
                break

        save_history(results, exp_dir)
    save_summary(results, exp_dir)
