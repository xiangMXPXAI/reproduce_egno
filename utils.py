...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, master_worker=True, checkpoint=None):

        score = -val_loss

        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, master_worker, checkpoint)
    ...
    def save_checkpoint(self, val_loss, model, master_worker=True, checkpoint=None):
        '''Saves model when validation loss decrease.'''
        if not master_worker:
            return
        if self.verbose and master_worker:
            self.trace_func(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
        torch.save(model.state_dict() if checkpoint is None else checkpoint, self.path)
